...
In order to keep the analysis unbiased from artificial cut tunings some data histogram bins are blinded. Following the advice in the top group, the blinding threshold is quantified as S/B reaching 0.1 $\sigma$ when assuming $B(t\rightarrow Hq)$ is 0.1\% which is the limit set by the 36fb$^{-1}$ combined results.
