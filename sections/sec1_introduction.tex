...
\label{sec:SRs}
As demonstrated in the Figure \ref{fig:diagrams}, depending on the production modes and the decay of the $W$ boson from top quark, the analysis is split into 4 kinds of final states as shown in table \ref{tab:signalevents}. The $\Htautau$ can be identified either in $\tlhad$ or $\thadhad$ final states when the $W$ decays hadronically and only in  $\thadhad$ when the $W$ decay leptonically.

Due to the low statistics in STL and TTL regions, the STL and TTL are merged into a single region $l\thadhad$ for $H\to\thadhad$ where there is no light jet multiplicity requirement and the two $\tauhad$ are required to have the same charge. However due to the low tau reconstruction rate, it is not rare that one of the taus fails the reconstruction and remains as a jet or goes missing. So the $l\thad$ 1j and $l\thad$ 2j are included as signal regions where the lepton and $\thad$ are required to have the same charged to reduce background. For the STH/TTH channels, the two tau candidates (including both $\taulep$ and $\tauhad$) are required to have opposite charges.  
