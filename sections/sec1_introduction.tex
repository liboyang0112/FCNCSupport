...
\end{equation}
%
Where the operator notation is consistent with \cite{fcnc_production_theory}. The coefficient $C_{u\phi}$'s can be extracted as follows according to the Madgraph calculation using TopFCNC UFO:
\begin{equation}
\begin{array}{l}
...
To give a better impression on the numbers, we use $\text{BR}(t\to qH)=0.2\%$ as benchmark, which is corresponding to ($C^{13}_{u\phi})^2 + (C^{31}_{u\phi})^2=3.89$, $\sigma(ug\to tH)=1421.8$ pb, $\sigma(cg\to tH)=206.0$ pb.
...
In order to search for the FCNC coupling, various signal regions will be defined as shown in \ref{sec:SRs}. The background modelling will be focused on the fake taus.

BDT models are trained against main background in order to separate signal and backgrounds in each signal region.
...
\label{sec:SRs}
As shown in the Figure \ref{fig:diagrams}, depending on the production modes and the decay of the $W$ boson from the SM top-quark decay, the signal is split into 4 kinds of final states as shown in table \ref{tab:signalevents}. The $\Htautau$ decay are required to be either in $\tlhad$ or $\thadhad$ final states when the $W$ decays hadronically and only in $\thadhad$ when the $W$ decay leptonically. The decay mode $\tll$ is dropped because this final state will be considered in the multi-lepton analysis. The mode $\tlhad$ is dropped when $W$ decay leptonically because in this case, the lepton fake will be significant when both leptons have the same sign of charges, the overlap of the fake lepton and fake taus can be quite hard to solve due to the low statistics.

Due to the low statistics in STL and TTL regions, the STL and TTL are merged into a single region $l\thadhad$ for $H\to\thadhad$ where there is no light jet multiplicity requirement and the two $\tauhad$ are required to have the opposite charge. However due to the low tau reconstruction efficiency, it is not rare that one of the taus fails the reconstruction and remains as a jet or goes missing. So the $l\thad$ 1j and $l\thad$ 2j are also included as signal regions where the lepton and $\thad$ are required to have the same sign of charges to reduce $t\bar t$ background. For the STH/TTH channels, the two tau candidates (including both $\taulep$ and $\tauhad$) are required to have opposite charges.  
...
The main background contributing to the leptonic channels, defined in the last section, is top pair production as shown in Figure \ref{fig:intro_pt_raw}. It can either contribute as reducible and irreducible background.

\input{\FCNCFigures/tex/intro_pt_raw}

As irreducible background, when the top pair decays full-leptonically to a $l$ and a $\tau$, and one of the $b$-jets from top decay fails the b-tagging, the event ends up in the STH $\tlhad$ region. If there are radiation or pile-up jets, the event could end up in the TTH $\tlhad$ region.
This irreducible background is modelled directly by Monte Carlo.

As reducible background, when the top pair decays full-leptonically, to a $l$ and a $\tauhad$, and one of the $b$-jets from top decay fails the b-tagging, if the $b$-jet or one of the radiation jets is reconstructed as a $\tauhad$, the event could end up in the $l\thadhad$ region if two $\tauhad$ have the same sign of charges. When the top pair decays semi-leptonically, and one of jets fakes a $\tauhad$, the event could end up in STH $\tlhad$ or TTH $\tlhad$ if the fake tau has the opposite charge to the lepton or in $l\tauhad$ 1j or $l\tauhad$ 2j if the fake tau has the same charge as the lepton. 

The reducible background except QCD multi-jets fakes is modelled by Monte Carlo but with scale factors derived from the dedicated $t\bar{t}$ control regions using the
dilepton events and the double-btagged lepton-jets events listed
in Table \ref{tab:sfcr} and explained in \ref{sec:sf_method}.
...
\multirow{6}{*}{CR}&$2l1tau1bnj$	& 1     & any				& 2      & 1			 & $ll$ OS\\ \cline{2-7}
&$2l1tau2bnj$	& 2     & any				& 2      & 1			 & $ll$ OS\\ \cline{2-7}
&$1l1tau2b2j SS$	& 2     & 2 				& 1      & 1             & $l\tauhad$ SS\\ \cline{2-7}
&$1l1tau2b2j OS$	& 2     & 2      			& 1      & 1             & $l\tauhad$ OS\\ \cline{2-7}
&$1l1tau2b3j SS$	& 2     & $\ge3$ 			& 1      & 1             & $l\tauhad$ SS\\ \cline{2-7}
&$1l1tau2b3j OS$	& 2     & $\ge3$      		& 1      & 1             & $l\tauhad$ OS\\ \hline
\multirow{5}{*}{SR}&$l\tauhad$ 2j  & 1     & 2					& 1      & 1			 & $l\tauhad$ SS\\ \cline{2-7}
&$l\tauhad$ 1j  & 1     & 1					& 1      & 1			 & $l\tauhad$ SS\\ \cline{2-7}
&$l\thadhad$	   & 1     & any				& 1      & 2             & $\thadhad$ OS\\ \cline{2-7}
&STH $\tlhad$   & 1     & 2      			& 1      & 1             & $\tlhad$ OS\\ \cline{2-7}
&TTH $\tlhad$   & 1     & $\ge3$ 			& 1      & 1             & $\tlhad$ OS\\ \hline
\end{tabular}
\end{table}

The QCD multi-jets also contribute a small fraction of the reducible background by faking both lepton and taus, especially in the low jet multiplicity region $l\tauhad$ 1j. This is modelled by the ABCD method by cutting on $\met$ and the PLV defined in Sec. \ref{sec:Plv} as explained in \ref{sec:ABCD}.

\subsection{Analysis strategy for in Hadronic channels}

Similar to STH $\tlhad$ or TTH $\tlhad$, the $t\bar{t}$ also contributes to the hadronic channels as reducible and irreducible background. But in the hadronic channels, much larger contribution is observed both from $Z\to \tau\tau$ plus $b$-jet and QCD multi-jets  with double fake taus as the excess of events shown in Figure \ref{fig:intro_os_pre_hadhad}. 

\input{\FCNCFigures/tex/intro_os_pre_hadhad}

The irreducible background is modelled directly by Monte Carlo. The reducible background with sub-leading taus faked is modelled by Fake Factor (FF) method using the
...
In the leptonic channels, the taus passing the RNN Loose working point are considered in the overlap removal procedure and missing transverse energy calculation, following the ttW multi-lepton group \cite{ttHMLgroup}.

The OLR and MET calculation procedures in hadronic channels are more forgiving in terms of taus since there are constantly two hadronic taus, and in the fake estimation, the taus not passing the RNN Medium are used. This way we can benefit from better statistics. While in the leptonic channels, due to the super high statistics, we are not be able to save the events with only one loose tau due to the limitation of the computing resources, so the loose taus are not used in the leptonic channels, and the fakes are calibrated in the control regions with medium taus. 
