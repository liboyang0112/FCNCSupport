...
The signal topology and kinematics are different across all the channels. To maximize the overall sensitivity, separated BDTG trainings are applied to each signal region. In order to increase the statistics in the training, all signal $tHu$ and $tHc$ events are merged and used against the total background including
both real and fake $\tau$ contributions.
A number of variables as the BDT inputs are used to train and test events in each signal region for maximal signal acceptance and background rejection. They are listed in Table~\ref{tab:importance_tthML} and Table~\ref{tab:importance_xTFW}. 
