...
both real and fake $\tau$ contributions. Training the charm channel separately does not gain much sensitivity and that the BDT outputs are very similar for charm and up quark samples. The BDT distributions for different types of signal samples are comparied in Figure \ref{fig:overtrain_hadhad} and \ref{fig:overtrain_lephad}.
...
As a cross check, the comparisons between BDT distributions in testing samples, as well as the test even and test odd ROC curves, are shown in Figure \ref{fig:overtrain_hadhad}-\ref{fig:overtrain_lhadhad}.
