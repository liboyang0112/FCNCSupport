...
\begin{table}[htb]
\caption{The expected $95\%$ CL exclusion upper limits on signal ( $\mu=1\to~\mathcal{B}(t\to Hq)=0.1\%$ ) with HVV samples into account in the leptonic channel, all uncertainties included. tcH and tuH means the nominal FCNC Htautau signal samples, tcH\_all and tuH\_all also consider the contributions from FCNC multilepton signal samples.}
\label{tab:ml_limit}
\input{\FCNCTables/limits_merged_signal}
\end{table}

\begin{table}[htb]
\caption{The yields comparation between HVV samples and our nominal samples. tcH and tuH means the nominal FCNC Htautau signal samples; tcH\_ml and tuH\_ml are for the contributions from FCNC multilepton signal samples.}
\label{tab:ml_yields}
